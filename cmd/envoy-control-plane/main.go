...
func init() {
	...
	// Matches management-api's own ORCHESTRATION_MODE / default (see its main.go) —
	// controls whether bare backend hostnames get the K8s cross-namespace FQDN
	// suffix (see qualifyHostname). Docker Compose's embedded DNS only knows
	// plain service names, so applying that suffix there breaks every cluster's
	// DNS resolution (STRICT_DNS clusters end up with zero healthy endpoints —
	// "no healthy upstream" on every route, regardless of hostname).
	orchestrationMode = os.Getenv("ORCHESTRATION_MODE")
	if orchestrationMode == "" {
		orchestrationMode = "docker"
	}
}
...
func qualifyHostname(host string) string {
	...
	if orchestrationMode != "k8s" && orchestrationMode != "kubernetes" {
		return host // Docker Compose: plain service names resolve as-is via embedded DNS
	}
									...
									"http_filters": []map[string]interface{}{
										...
										// app handles by redirecting to its login view.
										{
											...
											"typed_config": map[string]interface{}{
												...
												// Buffer and forward the request body to auth-service so it
												// can run per-route OPA payload validation (generated by the
												// repo-onboarding agent) in addition to JWT checks. Routes with
												// no attached payload policy are unaffected — auth-service
												// only evaluates a policy when the matched route declares one.
